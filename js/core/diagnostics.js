export class Diagnostics {
  ...
  snapshot(state){return{version:"5.1.0-quality-trading",saveVersion:state?.version,date:state?`Y${state.year} D${state.day}`:"no state",colony:state?.contract?.colonyName,colonies:state?.portfolio?.colonies?.length??0,speed:state?.speed,status:state?.status,heartbeat:this.heartbeat,ticks:this.ticks,msSinceLastTick:this.lastTick?Date.now()-this.lastTick:null,activeScans:state?.scans?.length??0,queuedScans:state?.scanQueue?.length??0};}
