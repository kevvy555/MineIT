...
export class ResourceService {
  ...
  sustainableRate(tile){this.ensureRenewable(tile);return this.baseSiteOutput(tile.level||1)*this.renewableRateFactor(tile);}
  sitePotentialRate(tile){if(this.isRenewable(tile))return this.sustainableRate(tile)*clamp(Number(tile.harvestIntensity)||1,.25,2);return this.baseSiteOutput(tile.level||1)*this.finiteRateFactor(tile);}
  ...
  collectionRate(state,tile){let rate=this.unthrottledCollectionRate(state,tile);if(tile.type==="food"||tile.type==="fuel")rate*=state.metrics.workforceSurvivalFactor??1;else rate*=(state.metrics.workforceCommercialFactor??1)*(state.metrics.industryCommercialFactor??1);return rate;}
