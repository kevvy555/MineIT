...
export class ResourceService {
  ...
  collectionRate(state,tile){if(!tile?.resourceId||tile.resourceCovered)return 0;let rate=this.unthrottledCollectionRate(state,tile);if(tile.type==="food"||tile.type==="fuel")rate*=state.metrics.workforceSurvivalFactor??1;else rate*=(state.metrics.workforceCommercialFactor??1)*(state.metrics.industryCommercialFactor??1);const powerFactors=state.metrics?.powerFactors||{},id=String(tile.id||tile.key||`${tile.x??0},${tile.y??0}`);rate*=Math.max(0,Math.min(1,Number(powerFactors[id]??1)));rate*=Math.max(0,Math.min(1.15,Number(state.metrics?.commandEfficiency??1)));return rate;}
