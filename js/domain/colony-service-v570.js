...
export class ColonyService extends LegacyColonyService{
  ...
  demand(state){
    const totals=this.totals(state);if(state.status==="dead")return{powerDemand:0,capacity:totals.power,capacityFactor:1,foodDemand:0,fuelDemand:0,oreDemand:0,staffFactor:0,populationRequired:this.industryPopulationRequirement(state),supportLoad:this.supportLoad(state)};
