...
export class LandService{
  ...
  syncExtraction(tile,addedBuild=0){
    ...
    const family=this.extractionFamily(tile),existing=tile.development?.kind==="extract"?tile.development:{kind:"extract",family,level:tile.level||1,investedBuild:0};existing.family=family;existing.level=Math.max(1,Math.min(5,Number(tile.level)||Number(existing.level)||1));existing.investedBuild=Math.max(0,Number(existing.investedBuild)||0)+Math.max(0,Number(addedBuild)||0);tile.level=existing.level;tile.development=existing;tile.resourceCovered=false;
  }
