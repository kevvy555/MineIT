...
export class WorldService {
  ...
  get(state,x,y){const key=tileKey(x,y);return state.tiles[key]||=({x,y,terrain:"plain",terrainVariant:1,revealed:false,lastScannedAtLevel:0,developed:false,level:0,depleted:false,resourceExhausted:false,development:null});}
  ...
  clearResult(tile,scanLevel=1){Object.assign(tile,{revealed:true,lastScannedAtLevel:Math.max(1,Number(scanLevel)||1),unresolved:false,unresolvedScanningLevel:0,requiredScanningLevel:0,empty:true,type:null,family:null,resourceId:null,name:"Clear Land",quality:null,resourceRarity:null,resourceMult:null,requiredMiningLevel:0,requiredMiningTech:null,terrainYieldFactor:null,sustainability:null,abundance:null,abundanceLabel:null,depositScale:null,reserve:null,initialReserve:null,renewableOriginalRank:null,renewableHealth:null,renewableWiped:false,harvestIntensity:null,depleted:false,resourceCovered:false});return tile;}
  applyTruth(tile,truth,scanLevel){Object.assign(tile,truth,{revealed:true,lastScannedAtLevel:Math.max(1,Number(scanLevel)||1),unresolved:false,unresolvedScanningLevel:0,empty:false,resourceCovered:!!tile.development&&tile.development.kind!=="extract"});return tile;}
  revealLegacy(state,x,y,tile=this.get(state,x,y),scanLevel=this.scanningLevel(state)){if(tile.resourceExhausted)return this.clearResult(tile,scanLevel);if(tile.revealed&&tile.resourceId){tile.lastScannedAtLevel=Math.max(Number(tile.lastScannedAtLevel)||0,scanLevel);tile.unresolved=false;tile.unresolvedScanningLevel=0;return tile;}const truth=this.legacyTruth(state,x,y,tile);return !truth||scanLevel<truth.requiredScanningLevel?this.clearResult(tile,scanLevel):this.applyTruth(tile,truth,scanLevel);}
  reveal(state,x,y,scanLevel=this.scanningLevel(state)){
    const tile=this.get(state,x,y),level=Math.max(1,Number(scanLevel)||1);if(tile.resourceExhausted)return this.clearResult(tile,level);if(tile.revealed&&Number(tile.lastScannedAtLevel)>=level)return tile;if(tile.revealed&&tile.resourceId){tile.lastScannedAtLevel=level;tile.unresolved=false;tile.unresolvedScanningLevel=0;tile.resourceCovered=!!tile.development&&tile.development.kind!=="extract";return tile;}if(!this.land)return this.revealLegacy(state,x,y,tile,level);const truth=this.resourceTruth(state,x,y,tile);return !truth||level<truth.requiredScanningLevel?this.clearResult(tile,level):this.applyTruth(tile,truth,level);
  }
  hint(state,x,y){
