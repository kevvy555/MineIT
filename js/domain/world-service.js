...
export class WorldService {
  ...
  unresolved(tile,def){const required=this.scanningRequirement(def);Object.assign(tile,{revealed:false,unresolved:true,unresolvedScanningLevel:required,requiredScanningLevel:required,empty:false,type:null,family:null,resourceId:null,name:"Unresolved Subsurface Anomaly",quality:null,resourceRarity:null,resourceMult:null,requiredMiningLevel:0,requiredMiningTech:null,sustainability:null,reserve:null,initialReserve:null,renewableOriginalRank:null,renewableHealth:null,renewableWiped:false,harvestIntensity:null,depleted:false});return tile;}
  clearResult(tile,extra={}){Object.assign(tile,{revealed:true,unresolved:false,unresolvedScanningLevel:0,requiredScanningLevel:0,empty:true,type:null,family:null,resourceId:null,name:"Clear Land",quality:null,resourceRarity:null,resourceMult:null,requiredMiningLevel:0,requiredMiningTech:null,sustainability:null,reserve:null,initialReserve:null,renewableOriginalRank:null,renewableHealth:null,renewableWiped:false,harvestIntensity:null},extra);return tile;}
  revealLegacy(state,x,y,tile=this.get(state,x,y)){
    if(tile.revealed&&!tile.unresolved)return tile;
    const random=seededRandom(this.legacySeed(state,x,y)),{a}=this.legacyFamilyWeights(state),type=this.legacyFamilyFor(state,random),def=this.resources.pick(RESOURCE_TYPES[type],random),requiredScanningLevel=this.scanningRequirement(def);if(this.scanningLevel(state)<requiredScanningLevel)return this.unresolved(tile,def);let quality=this.resources.quality(random,def.qualityBias,a.rare);quality=clamp(Math.round(quality),1,10000);
    const shared={revealed:true,unresolved:false,unresolvedScanningLevel:0,empty:false,type,family:type,resourceId:def.id,name:def.name,resourceRarity:def.rarity,resourceMult:def.multiplier,quality,requiredScanningLevel,requiredMiningLevel:def.miningLevel,requiredMiningTech:def.unlock,terrainYieldFactor:1,deepResource:null,deepRevealed:false};
  ...
  reveal(state,x,y){
    const tile=this.get(state,x,y);if(tile.revealed&&!tile.unresolved)return tile;if(!this.land)return this.revealLegacy(state,x,y,tile);
    ...
    if(random()>(SURFACE_CHANCE[tile.terrain]??.5))return this.clearResult(tile);
    const picked=this.pickSurface(state,tile,random);if(!picked)return this.clearResult(tile);
    const{type,def}=picked,requiredScanningLevel=this.scanningRequirement(def);if(this.scanningLevel(state)<requiredScanningLevel)return this.unresolved(tile,def);let quality=this.resources.quality(random,def.qualityBias,a.rare);quality=clamp(Math.round(quality),1,10000);const terrainYieldFactor=this.land.terrainYieldFactor({...tile,type,resourceId:def.id})||1,shared={revealed:true,unresolved:false,unresolvedScanningLevel:0,empty:false,type,family:type,resourceId:def.id,name:def.name,resourceRarity:def.rarity,resourceMult:def.multiplier,quality,requiredScanningLevel,requiredMiningLevel:def.miningLevel,requiredMiningTech:def.unlock,terrainYieldFactor};
    if(def.renewable){const abundance=this.renewableAbundance(random),rank=this.resources.renewableRank(abundance.label);Object.assign(tile,shared,{sustainability:"renewable",abundance:abundance.factor,abundanceLabel:abundance.label,renewableOriginalRank:rank,renewableHealth:rank+1,renewableWiped:false,harvestIntensity:1,reserve:null,initialReserve:null,depleted:false});return tile;}
    const deposit=this.finiteDeposit(random,a.res,type),baseline=this.resources.baseSiteOutput(1)*this.resources.finiteRateFactor(deposit.label)*terrainYieldFactor,reserve=Math.max(1,Math.round(baseline*360*deposit.years));Object.assign(tile,shared,{sustainability:"finite",abundance:1,depositScale:deposit.label,reserve,initialReserve:reserve,renewableOriginalRank:null,renewableHealth:null,renewableWiped:false,harvestIntensity:null});return tile;
  }
  hint(state,x,y){
    const tile=this.get(state,x,y),required=Math.max(0,Number(tile.unresolvedScanningLevel)||0);if(tile.unresolved&&required>this.scanningLevel(state))return`Unresolved anomaly • Scanning L${required} required`;if(tile.unresolved)return"Anomaly can now be resolved • survey again";
