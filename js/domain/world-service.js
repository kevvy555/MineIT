...
export class WorldService {
  ...
  reveal(state,x,y,scanLevel=this.scanningLevel(state)){
    const tile=this.get(state,x,y),level=Math.max(1,Number(scanLevel)||1);if(tile.revealed&&Number(tile.lastScannedAtLevel)>=level)return tile;if(tile.revealed&&tile.resourceId){tile.lastScannedAtLevel=level;tile.unresolved=false;tile.unresolvedScanningLevel=0;tile.resourceCovered=!!tile.development&&tile.development.kind!=="extract";return tile;}if(!this.land)return this.revealLegacy(state,x,y,tile,level);const truth=this.resourceTruth(state,x,y,tile);return !truth||level<truth.requiredScanningLevel?this.clearResult(tile,level):this.applyTruth(tile,truth,level);
  }
  hint(state,x,y){
