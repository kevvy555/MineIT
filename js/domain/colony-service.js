...
export class ColonyService{
  ...
  industryPopulationRequirement(installedOrState){const isState=installedOrState&&typeof installedOrState==="object",installed=isState?this.totals(installedOrState).builtIndustry:Math.max(0,Number(installedOrState)||0),efficiency=isState?(installedOrState.metrics?.industryWorkforceEfficiency||1):1;return installed>0?Math.max(40,Math.round(installed*.8*efficiency)):0;}
  industryPopulationFactor(state){const required=this.industryPopulationRequirement(state);return required>0?clamp(this.planetaryResidentCount(state)/required,0,1):1;}
  ...
  industryOperationalCapacity(state){if(state.status==="dead"||state.colony?.emergencyMode)return 0;const totals=this.totals(state);return Math.max(0,totals.shipIndustry+totals.builtIndustry*this.industryPopulationFactor(state));}
  industryOperationalFactor(state){const totals=this.totals(state),installed=Math.max(0,totals.industry);return installed>0?clamp(this.industryOperationalCapacity(state)/installed,0,1):1;}
  ...
  demand(state,{fuelStock=null}={}){const network=this.powerNetwork(state,{fuelStock}),totals=network.totals,builtStaffFactor=this.industryPopulationFactor(state),staffFactor=this.industryOperationalFactor(state),populationRequired=this.industryPopulationRequirement(state),fuelDemand=network.fullFuelBurn,oreEfficiency=state.metrics.industryOreEfficiency||1,poweredIndustry=state.colony?.emergencyMode?0:totals.shipIndustry+totals.builtIndustry*builtStaffFactor*(network.industryPowerFactor??1),oreDemand=poweredIndustry>0?Math.max(.05,poweredIndustry*(CONFIG.ORE_PER_INDUSTRY_LEVEL/100)*oreEfficiency):0;return{powerDemand:network.demand,capacity:totals.power,capacityFactor:network.availableGeneration>0?clamp(network.deliveredGeneration/Math.max(.1,network.demand),0,1):0,foodDemand:this.foodDemandForPopulation(state),fuelDemand,oreDemand,staffFactor,builtStaffFactor,populationRequired,supportLoad:this.supportLoad(state),installedIndustry:totals.industry,shipIndustry:totals.shipIndustry,builtIndustry:totals.builtIndustry,powerNetwork:network,fullFuelBurn:network.fullFuelBurn,fuelLimitedGeneration:network.fuelLimitedGeneration,deliveredGeneration:network.deliveredGeneration};}
