...
export class ColonyService {
  ...
  expandHousing(state){const check=this.canExpandHousing(state);if(!check.ok)return check;this.inventory.consumeCategory(state,"build",check.build);state.company.cash-=check.cash;state.colony.housingLevel++;state.colony.housingCapacity=check.nextCap;return{ok:true,...check};}
  ...
  expandIndustry(state){const check=this.canExpandIndustry(state);if(!check.ok)return check;this.inventory.consumeCategory(state,"build",check.build);state.company.cash-=check.cash;state.colony.industryLevel=check.next;return{ok:true,...check};}
