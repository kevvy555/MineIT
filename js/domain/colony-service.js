...
export class ColonyService{
  ...
  planetaryResidentCount(state){const assignments=state.colony?.shipAccommodation||{};const aboard=Object.values(assignments).reduce((sum,value)=>sum+Math.max(0,Math.floor(Number(value)||0)),0);return Math.max(0,Math.floor(Number(state.pop)||0)-aboard);}
  ...
  headquartersRows(state){return localBuildings(state,"headquarters").map(tile=>{const id=tileId(state,tile),level=buildingLevel(tile.development),requiredStaff=tierValue(HEADQUARTERS_MINIMUM_STAFF,level);return{id,tile,level,requiredStaff,requiredPower:tierValue(HEADQUARTERS_POWER_DEMAND,level),constructed:tile.development?.constructionComplete!==false};}).sort((a,b)=>a.id.localeCompare(b.id));}
  ...
  commandLoad(state){
    let load=0;for(const tile of Object.values(state.tiles||{})){const dev=tile.development;if(!dev||tile.depleted||dev.kind==="headquarters")continue;let weight=0;if(dev.kind==="housing")weight=HEADQUARTERS_COMMAND_LOAD.housing;else if(dev.kind==="power")weight=HEADQUARTERS_COMMAND_LOAD.power;else if(dev.kind==="industry")weight=HEADQUARTERS_COMMAND_LOAD.industry;else if(dev.kind==="extract")weight=HEADQUARTERS_COMMAND_LOAD[familyFor(tile)]||HEADQUARTERS_COMMAND_LOAD.ore;load+=weight*tierValue([1,2,3,4,5],dev.level);}
    return load;
  }
  commandCapableShips(state){const ships=state.company?.expansion?.ships||[];return ships.filter(ship=>ship.status==="docked"&&ship.colonyId===state.colonyId&&(ship.commandCapable===true||ship.source==="charter-issued"||String(ship.shipClassId||"").includes("colony"))&&Number(ship.crew||0)>=Math.max(0,Number(ship.minimumCrew)||0));}
  powerDemandBreakdown(state){
    const totals=this.totals(state),staffFactor=this.industryPopulationFactor(state),planetary=this.planetaryResidentCount(state),supportLoad=this.supportLoad(state),emergency=!!state.colony?.emergencyMode;
    const hq=this.headquartersRows(state).filter(row=>row.constructed).map(row=>({id:row.id,tile:row.tile,requested:row.requiredPower,priority:"headquarters",binary:true}));
    ...
    const industry=[];for(const tile of localBuildings(state,"industry")){if(tile.development?.productionStopped)continue;const level=buildingLevel(tile.development),idle=tierValue(INDUSTRY_IDLE_POWER,level),capacity=buildingCapacity("industry",level),variable=emergency?0:capacity*staffFactor*INDUSTRY_VARIABLE_POWER_PER_CAPACITY;industry.push({id:tileId(state,tile),tile,requested:idle+variable,idle,variable,priority:"commercial",binary:false});}
    const sites=[];for(const tile of this.workforceSites(state)){const requested=tierValue(FACILITY_POWER_DEMAND[familyFor(tile)]||FACILITY_POWER_DEMAND.mine,tile.level);sites.push({id:tileId(state,tile),tile,requested,priority:["food","fuel"].includes(tile.type)?"survival":"commercial",binary:false});}
    return{totals,headquarters:hq,lifeSupport:lifeSupport+housing.reduce((s,row)=>s+row.requested,0),housing,industry,sites,spaceport:{id:"spaceport",requested:BASIC_SPACEPORT_POWER,priority:"spaceport",binary:true},planetaryResidents:planetary,staffFactor,requested:hq.reduce((s,r)=>s+r.requested,0)+lifeSupport+housing.reduce((s,r)=>s+r.requested,0)+industry.reduce((s,r)=>s+r.requested,0)+sites.reduce((s,r)=>s+r.requested,0)+BASIC_SPACEPORT_POWER};
  }
  powerNetwork(state,{fuelStock=null}={}){
    ...
    const survival=allocateProportional("survival",[...b.sites.filter(row=>row.priority==="survival"),...b.industry.filter(row=>false)],remaining);remaining-=survival.used;
    ...
    const commercial=allocateProportional("commercial",[...b.industry,...b.sites.filter(row=>row.priority==="commercial")],remaining);remaining-=commercial.used;
    ...
    const demand=b.requested,unused=Math.max(0,remaining),totalDelivered=available-unused,bandRows=["headquarters","life-support","survival","spaceport","commercial"].map(priority=>{const requested=rows.filter(row=>row.priority===priority).reduce((s,r)=>s+r.requested,0),got=rows.filter(row=>row.priority===priority).reduce((s,r)=>s+r.delivered,0);return{priority,requested,delivered:got,shortage:Math.max(0,requested-got),factor:requested>0?got/requested:1};});
    return{...b,onlineCapacity,fuelRatio,fullFuelBurn,fuelLimitedGeneration:available,availableGeneration:available,deliveredGeneration:totalDelivered,unusedGeneration:unused,shortage:Math.max(0,demand-totalDelivered),rows,bandRows,demand,generated:totalDelivered};
  }
  headquartersNetwork(state,{fuelStock=null}={}){
    const staffing=this.headquartersStaffing(state),power=this.powerNetwork(state,{fuelStock}),rows=staffing.rows.map(row=>{const delivered=power.headquarters.find(item=>item.id===row.id)?.delivered||0;return{...row,powered:row.constructed&&delivered>=row.requiredPower&&row.requiredPower>0};});
    ...
    return{...staffing,rows,power,primary,primaryOperational,source,activeHeadquarters:active,capacity:hqCapacity,load,overloadRatio:overload,overloadPenalty:penalty,bonus,efficiency,commandEfficiency:efficiency,disconnected:!source};
  }
  commandStatus(state,options={}){return this.headquartersNetwork(state,options);}
  setPrimaryHeadquarters(state,tile){
    const id=tileId(state,tile),network=this.headquartersNetwork(state);const row=network.rows.find(item=>item.id===id);
  ...
  workforceAvailable(state){return Math.max(0,this.baseWorkforceAvailable(state)-this.headquartersStaffing(state).reserved);}
  ...
  sitePowerDemand(tile){const level=Math.max(1,Math.min(5,Math.round(Number(tile?.level)||1)));return tierValue(SITE_POWER,level);}
  ...
  foodDemandForPopulation(state,population=null){const residents=population===null?this.planetaryResidentCount(state):Math.max(0,Number(population)||0);return state.status==="dead"?0:Math.max(.1,residents*CONFIG.FOOD_PER_COLONIST);}
