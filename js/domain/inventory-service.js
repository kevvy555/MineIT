import { CATEGORY_NAMES } from "../data/resources.js?v=5.3.0";
import { DEFAULT_QUALITY_BAND } from "./resource-service.js?v=5.3.0";
...
export class InventoryService {
  ...
  syncEntry(entry){
    if(!entry.qualityBands){const legacy=Math.max(0,Number(entry.amount)||0);entry.qualityBands={};if(legacy)entry.qualityBands[DEFAULT_QUALITY_BAND]={amount:legacy};}
    let total=0;for(const [key,band] of Object.entries(entry.qualityBands)){const info=this.resources.qualityBandByKey(key);band.key=info.key;band.label=info.label;band.className=info.className;band.multiplier=info.multiplier;band.min=info.min;band.max=info.max;band.amount=Math.max(0,Number(band.amount)||0);total+=band.amount;}
    entry.amount=total;return entry;
  }
  ensureEntry(state,type,resourceId,name){
    state.inventory||={};const key=this.key(type,resourceId);
    if(!state.inventory[key]){const def=this.resources.get(type,resourceId);state.inventory[key]={key,type,resourceId,name:name||def?.name||resourceId,category:this.categoryName(type),amount:0,qualityBands:{}};}
    const entry=state.inventory[key];entry.key=key;entry.type=type;entry.resourceId=resourceId;entry.name=name||entry.name||this.resources.get(type,resourceId)?.name||resourceId;entry.category=this.categoryName(type);return this.syncEntry(entry);
  }
  ...
  consumeCategory(state,type,requested){
    const original=Math.max(0,Number(requested)||0);let remaining=original,consumed=0;
    const lots=[];for(const entry of this.stock(state).filter(e=>e.type===type)){for(const band of this.bands(entry))lots.push({entry,band,unitValue:this.resources.sellPrice(entry.type,entry.resourceId,band.key)});}
    lots.sort((a,b)=>a.unitValue-b.unitValue||a.band.min-b.band.min);
    for(const lot of lots){if(remaining<=0)break;const take=Math.min(lot.band.amount,remaining);lot.band.amount-=take;remaining-=take;consumed+=take;this.syncEntry(lot.entry);}
    return{requested:original,consumed,ratio:original>0?Math.min(1,consumed/original):1};
  }
}
