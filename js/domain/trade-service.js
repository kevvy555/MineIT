...
export class TradeService {
  ...
  arrive(state){if(state.trade.active)return false;state.trade.active=true;state.trade.returnSpeed=state.speed>0?state.speed:1;state.trade.arrivedAt=this.absoluteDay(state);state.trade.visits++;state.trade.nextArrivalDay+=CONFIG.TRADE_INTERVAL_DAYS;state.speed=0;return true;}
  depart(state){if(!state.trade.active)return false;state.trade.active=false;state.trade.arrivedAt=null;if(state.status==="playing")state.speed=state.trade.returnSpeed||1;return true;}
  daysUntilArrival(state){return state.trade.active?0:Math.max(0,state.trade.nextArrivalDay-this.absoluteDay(state));}
  sell(state,key,amount){if(!state.trade.active)return{ok:false,reason:"No corporate ship is docked."};const entry=state.inventory[key];if(!entry||entry.amount<=0)return{ok:false,reason:"No stock available."};const qty=Math.min(entry.amount,Math.max(0,Number(amount)||0));if(qty<=0)return{ok:false,reason:"Nothing selected."};const revenue=qty*this.sellPrice(entry.type,entry.resourceId);entry.amount-=qty;state.company.cash+=revenue;state.company.earn+=revenue;return{ok:true,qty,revenue,entry};}
  ...
  buy(state,key,amount){if(!state.trade.active)return{ok:false,reason:"No corporate ship is docked."};const item=this.catalog().find(x=>x.key===key);if(!item)return{ok:false,reason:"Unknown resource."};const requested=Math.max(0,Number(amount)||0);if(!requested)return{ok:false,reason:"Nothing selected."};const price=this.buyPrice(item),affordable=Math.floor(state.company.cash/price),qty=Math.min(requested,affordable);if(qty<=0)return{ok:false,reason:"Insufficient cash."};const cost=qty*price,entry=this.inventory.ensureEntry(state,item.type,item.resourceId,item.name);entry.amount+=qty;state.company.cash-=cost;return{ok:true,qty,cost,entry};}
}
