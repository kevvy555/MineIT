...
export class TradeService {
  ...
  colonyTradeReserve(state){state.colony||={};if(Number.isFinite(Number(state.colony.tradeReserve)))return Math.max(0,Math.floor(Number(state.colony.tradeReserve)||0));const legacy=Object.values(state.colony.tradeReserves||{}).reduce((max,value)=>Math.max(max,Math.max(0,Number(value)||0)),0);state.colony.tradeReserve=Math.floor(legacy);return state.colony.tradeReserve;}
  setColonyTradeReserve(state,amount){state.colony||={};state.colony.tradeReserve=Math.max(0,Math.floor(Number(amount)||0));delete state.colony.tradeReserves;return state.colony.tradeReserve;}
  tradeReserve(state,key=null){return this.colonyTradeReserve(state);}
  reserveShortfall(state,key){const entry=state.inventory?.[key],stock=entry?this.inventory.syncEntry(entry).amount:0;return Math.max(0,this.colonyTradeReserve(state)-stock);}
  sellableAmount(state,entry){return Math.max(0,this.inventory.syncEntry(entry).amount-this.colonyTradeReserve(state));}
  processingBonus(state){return Math.max(0,Math.min(CONFIG.INDUSTRY_PROCESSING_MAX_BONUS,Number(state?.metrics?.processingBonus)||0));}
  processingMultiplier(state){return 1+this.processingBonus(state);}
  qualityRows(entry,state=null){return this.inventory.bands(entry).map(band=>{const unitPrice=state?this.exportPrice(state,entry.type,entry.resourceId,band.key):this.sellPrice(entry.type,entry.resourceId,band.key);return{...band,unitPrice,value:band.amount*unitPrice};});}
  entryValue(state,entry){return this.qualityRows(entry,state).reduce((sum,row)=>sum+row.value,0);}
  entrySellableValue(state,entry){let remaining=this.sellableAmount(state,entry),value=0;for(const row of this.qualityRows(entry,state).sort((a,b)=>b.unitPrice-a.unitPrice)){if(remaining<=0)break;const take=Math.min(remaining,row.amount);remaining-=take;value+=take*row.unitPrice;}return value;}
  sellableStock(state){return this.stock(state).map(entry=>({...entry,reserve:this.colonyTradeReserve(state),sellableAmount:this.sellableAmount(state,entry),saleValue:this.entrySellableValue(state,entry)})).filter(entry=>entry.sellableAmount>.0001).sort((a,b)=>b.saleValue-a.saleValue||b.sellableAmount-a.sellableAmount||a.name.localeCompare(b.name));}
  quoteSell(state,key,amount){const entry=state.inventory?.[key];if(!entry)return{qty:0,revenue:0};let remaining=Math.min(Math.max(0,Number(amount)||0),this.sellableAmount(state,entry),this.exportRemaining(state)),qty=0,revenue=0;for(const row of this.qualityRows(entry,state).sort((a,b)=>b.unitPrice-a.unitPrice)){if(remaining<=0)break;const take=Math.min(remaining,row.amount);remaining-=take;qty+=take;revenue+=take*row.unitPrice;}return{qty,revenue};}
  sellQuoteForEntries(state,entries){const allowed=new Map(),lots=[];for(const entry of entries){allowed.set(entry.key,this.sellableAmount(state,entry));for(const band of this.qualityRows(entry,state))lots.push({entry,band});}lots.sort((a,b)=>b.band.unitPrice-a.band.unitPrice||b.band.value-a.band.value);let room=this.exportRemaining(state),qty=0,revenue=0;for(const {entry,band} of lots){if(room<=0)break;const can=Math.max(0,allowed.get(entry.key)||0),take=Math.min(room,can,band.amount);if(take<=0)continue;allowed.set(entry.key,can-take);room-=take;qty+=take;revenue+=take*band.unitPrice;}return{qty,revenue,exportRemaining:room};}
  sellAllQuote(state){return this.sellQuoteForEntries(state,this.stock(state));}
  sellCategoryQuote(state,type){return this.sellQuoteForEntries(state,this.stock(state).filter(entry=>entry.type===type));}
  ...
  shouldArrive(state){
    ...
    if(!hasFreeBerth(state)){state.trade.orbitalHolding=true;state.trade.orbitalSinceAbsoluteDay??=this.absoluteDay(state);return false;}
    return true;
  }
  arrive(state){
  ...
  sellCategory(state,type){return this.sellEntries(state,this.stock(state).filter(entry=>entry.type===type));}
