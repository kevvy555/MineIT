...
export class TransportService {
  ...
  canRequest(state,amount){const qty=Math.max(0,Math.floor(Number(amount)||0)),cost=this.cost(state,qty);if(state.status==="dead"||state.contract?.ended||state.status==="liability")return{ok:false,reason:"This colony cannot receive new colonists.",qty,cost};const network=this.networkStatus(state);if(!network.networkAvailable)return{ok:false,reason:"Conglomerate network offline: restore the Primary Headquarters before requesting a new dedicated transport.",qty,cost,network};if(qty<=0)return{ok:false,reason:"Choose at least one colonist.",qty,cost};const capacity=this.availableCapacity(state);if(qty>capacity)return{ok:false,reason:`Only ${capacity} supported places remain after pending transports.`,qty,cost,capacity};if((Number(state.company?.cash)||0)<cost)return{ok:false,reason:"Insufficient cash for dedicated transport.",qty,cost,capacity};return{ok:true,qty,cost,capacity,network};}
