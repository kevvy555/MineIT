...
export class ExpansionService{
  ...
  loadPassengers(state,...args){const parsed=parseShipArgs(args,1),[amount]=parsed.values,ship=this.ship(state,parsed.shipId);if(!ship||!this.isAtActiveColony(state,ship.id))return{ok:false,reason:"The selected player ship is not docked at this colony."};const powerReason=this.spaceportServiceFailure(state);if(powerReason)return{ok:false,reason:powerReason};const qty=Math.min(Math.max(0,Math.floor(Number(amount)||0)),this.travelResidentCount(state,ship.id),this.passengerRemaining(state,ship.id));if(qty<=0)return{ok:false,reason:"No passengers can be loaded."};this.removeResidentsForTravel(state,ship.id,qty);ship.passengers+=qty;return{ok:true,qty};}
  unloadPassengers(state,...args){const parsed=parseShipArgs(args,1),[amount=Infinity]=parsed.values,ship=this.ship(state,parsed.shipId);if(!ship||!this.isAtActiveColony(state,ship.id))return{ok:false,reason:"The selected player ship is not docked at this colony."};const powerReason=this.spaceportServiceFailure(state);if(powerReason)return{ok:false,reason:powerReason};const room=Math.max(0,this.accommodationCapacity(state,ship.id)-this.shipResidentCount(state,ship.id)),qty=Math.min(ship.passengers,room,Number.isFinite(amount)?Math.max(0,Math.floor(Number(amount)||0)):Infinity);if(qty<=0)return{ok:false,reason:room<=0?"Ship accommodation is full.":"No passengers aboard."};state.pop=Number(state.pop||0)+qty;ship.passengers-=qty;this.accommodationAssignments(state)[ship.id]=this.shipResidentCount(state,ship.id)+qty;return{ok:true,qty};}
