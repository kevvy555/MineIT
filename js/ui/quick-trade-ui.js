...
export class TradeUI extends BaseTradeUI{
  ...
  sellView(){
    ...
    return `<section class="trade-view-panel"><div class="trade-view-head"><div><strong>SELL COLONY STOCK</strong><small>Highest quality is sold first. Colony reserves are protected.</small></div><b>${formatMoney(allQuote.revenue)}</b></div>${this.amountControl("sell",this.sellAmount)}<div class="trade-quick-list">${rows.length?rows.map(entry=>{const q=this.trade.quoteSell(this.state,entry.key,this.sellAmount),reserve=this.trade.tradeReserve(this.state,entry.key);return`<div class="trade-quick-row"><div class="trade-row-copy"><strong>${entry.name}</strong><small>${formatNumber(entry.amount)} stock${reserve>0?` • <span class="reserve">${formatNumber(reserve)} reserve</span>`:""} • ${formatNumber(entry.sellableAmount)} sellable</small></div><button data-sell-key="${entry.key}" ${q.qty<=0?"disabled":""}>SELL${q.qty>0?`<span>${formatMoney(q.revenue)}</span>`:""}</button></div>`}).join(""):`<div class="trade-empty">No stock is available above your colony reserves.</div>`}</div><div class="trade-view-footer"><button data-sell-all class="trade-primary" ${allQuote.qty<=0?"disabled":""}>SELL ALL${allQuote.qty>0?` — ${formatMoney(allQuote.revenue)}`:""}</button>${this.pager("sell",this.sellPage,pages)}</div></section>`;
  }
