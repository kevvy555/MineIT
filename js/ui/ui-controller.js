...
export class UIController {
  ...
  dispose(){
    ...
    this.speedControls=null;this.speedClickHandler=null;this.speedInputBound=false;if(this.modal)this.modal.onkeydown=null;this.releaseInteractionDiagnostics();
  ...
  panelTitle(title,icon=""){return`<div class="panel-title"><div class="resource-title">${icon}<strong id="modalTitle">${title}</strong></div><button class="close" data-close aria-label="Close">✕</button></div>`;}
  open(title,body){const previousTitle=this.modal?.querySelector(".panel-title strong")?.textContent?.trim()||null,returnFocus=document.activeElement;this.traceInteraction("ui-open-start",null,{title,previousTitle,activePointers:this.interactionPointers.size});this.modal.innerHTML=`${this.panelTitle(title)}<div class="modal-body">${body}</div>`;this.modal.classList.remove("hidden","establishment-modal","full-screen-panel");this.modal.setAttribute("role","dialog");this.modal.setAttribute("aria-modal","true");this.modal.setAttribute("aria-labelledby","modalTitle");const close=this.modal.querySelector("[data-close]"),focusable=()=>[...this.modal.querySelectorAll('button:not([disabled]),[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')].filter(element=>!element.hidden);close.onclick=()=>{this.modal.classList.add("hidden");returnFocus?.focus?.();};this.modal.onkeydown=event=>{if(event.key==="Escape"){event.preventDefault();close.click();return;}if(event.key!=="Tab")return;const controls=focusable();if(!controls.length){event.preventDefault();return;}const first=controls[0],last=controls[controls.length-1];if(event.shiftKey&&(document.activeElement===first||!this.modal.contains(document.activeElement))){event.preventDefault();last.focus();}else if(!event.shiftKey&&(document.activeElement===last||!this.modal.contains(document.activeElement))){event.preventDefault();first.focus();}};close.focus();this.traceInteraction("ui-open-commit",null,{title,activePointers:this.interactionPointers.size});}
