...
export class UIController {
  ...
  bindInteractionDiagnostics(){
    ...
    const clickBubble=event=>{const control=this.interactionControl(event.target);if(!control)return;this.traceInteraction("click-bubble",control,{detail:event.detail,connectedAfterHandler:!!control.isConnected,modalTitleAfterHandler:this.modal?.querySelector(".panel-title strong")?.textContent?.trim()||null});};
