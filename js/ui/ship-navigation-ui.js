...
export class UIController extends PlayerShipUIController{
  constructor(opts){super(opts);this._shipNavActive=false;this._shipControlTitle="Ship Control";this._panelReturn=null;this.selectedStarSystemId=null;this.demolitionRevision=0;this.starMapRevision=0;this.shipControlRevision=0;}
  ...
  shipControlStatusLabel(ship){
    if(ship.status==="travelling")return{label:"TRAVELLING",cls:"transit"};
    if(ship.status==="orbiting")return{label:"ORBITING",cls:"warn"};
    if(ship.status==="arrived")return{label:"ARRIVED",cls:"warn"};
    if(ship.status==="home")return{label:"HOME",cls:""};
    if(ship.status==="docked")return{label:"DOCKED",cls:""};
    return{label:String(ship.status||"UNKNOWN").toUpperCase(),cls:"warn"};
  }

  async playerShipPanel(){
    this._shipNavActive=false;const returnTo=this._panelReturn;this._panelReturn=null;const revision=++this.shipControlRevision,ship=this.expansion.ship(this.state);
    if(!ship){this.starMap();return;}
    if(ship.status==="lost"){if(returnTo)this._panelReturn=returnTo;await this.lostShipPanel(revision,ship,"shipControlRevision");return;}
    const title=ship.name||"Ship Control";this._shipControlTitle=title;
    const dockedHere=ship.status==="docked"&&this.expansion.isAtActiveColony(this.state,ship.id),command=this.colony?.commandStatus?.(this.state),csmLinked=command?.source?.type==="ship"&&command.source.id===ship.id;
    const colonyName=this.state.contract?.colonyName||"Colony",klass=ship.shipClassName||ship.className||ship.role||"Player vessel",status=this.shipControlStatusLabel(ship);
    let body;try{body=await renderViewTemplate("./views/ship-control.html");}catch(error){if(revision!==this.shipControlRevision)return;this.diagnostics?.error?.("ship control template failed",error);this.toast("Unable to open Ship Control.");return;}
    if(revision!==this.shipControlRevision)return;
    if(returnTo)this._panelReturn=returnTo;
    this.open(title,body);this.modal.classList.add("player-ship-menu-modal","ship-control-modal","full-screen-panel");
    const root=this.modal.querySelector("[data-ship-control]");if(!root)return;
    const setText=(sel,value)=>{const node=root.querySelector(sel);if(node)node.textContent=String(value??"");};
    setText("[data-ship-class]",klass);setText("[data-ship-blurb]",csmLinked?"Ship systems only. Colony management rides the plugged-in Colony Support Module while this vessel holds colony command.":"Ship systems only. Colony management is not mixed into these tiles — it rides the Colony Support Module when installed and linked.");
    const badge=root.querySelector("[data-ship-status-badge]");if(badge){badge.className=`ship-control-status${status.cls?` ${status.cls}`:""}`;badge.textContent=status.label;}
    setText("[data-m-cargo]",`${formatNumber(this.expansion.cargoAmount(this.state,ship.id))} / ${formatNumber(ship.cargoCapacity)}`);
    setText("[data-m-fuel]",`${formatNumber(this.expansion.fuelAmount(this.state,ship.id))} / ${formatNumber(ship.fuelCapacity)}`);
    setText("[data-m-food]",`${formatNumber(this.expansion.foodAmount(this.state,ship.id))} / ${formatNumber(ship.foodCapacity)}`);
    setText("[data-m-crew]",`${formatNumber(ship.crew)} / ${formatNumber(ship.minimumCrew)}`);
    const bay=root.querySelector("[data-csm-bay]"),openCsm=root.querySelector("[data-csm-open]");
    if(csmLinked){bay?.classList.remove("offline");setText("[data-csm-title]","CSM-01 Colony Support Module");setText("[data-csm-text]","Hard-mounted colony command interface. Opens the same Colony Control surface used at Headquarters, linked to the managed colony.");setText("[data-csm-pill]","LINKED");root.querySelector("[data-csm-meta]").innerHTML=`<span>PORT A3 SEATED</span><span>MANAGING • ${esc(colonyName)}</span><span>COMMAND SOURCE • SHIP</span>`;if(openCsm){openCsm.disabled=false;openCsm.textContent="OPEN COLONY CONTROL";openCsm.onclick=()=>{this._panelReturn="ship";this.colonyControl?.();};}}
    else{bay?.classList.add("offline");const travelling=ship.status==="travelling"||ship.status==="orbiting"||ship.status==="arrived";setText("[data-csm-title]",travelling?"CSM BAY • NO COLONY LINK":"CSM BAY EMPTY / MODULE OFFLINE");setText("[data-csm-text]",travelling?"Module cannot hold a colony command link while the vessel is away from colony command.":"No active colony command link from this vessel. Colony Control is opened from Primary Headquarters on the map.");setText("[data-csm-pill]",travelling?"UNLINKED":"OFFLINE");root.querySelector("[data-csm-meta]").innerHTML=travelling?`<span>EN ROUTE</span><span>NO COLONY SESSION</span>`:`<span>PORT A3 IDLE</span><span>${command?.source?.type==="headquarters"?"HQ HOLDS COMMAND":"NO COMMAND LINK"}</span><span>SHIP-ONLY MODE</span>`;if(openCsm){openCsm.disabled=true;openCsm.textContent="COLONY CONTROL UNAVAILABLE";}}
    const target=ship.targetSystemId?this.expansion.system(this.state.company.expansion,ship.targetSystemId):null,profile=target?this.expansion.travelProfile(this.state,target.id,null,ship.id):null,fuel=this.expansion.fuelAmount(this.state,ship.id),food=this.expansion.transitFoodAmount?.(this.state,ship.id)??this.expansion.foodAmount(this.state,ship.id);
    setText("[data-route-dest]",target?.name||(ship.status==="orbiting"?"Orbital hold":"NOT SELECTED"));
    setText("[data-route-dist]",profile?`${profile.distanceLy.toFixed(1)} ly`:(ship.status==="orbiting"?"Orbital hold":"—"));
    setText("[data-route-eta]",profile?`${formatNumber(profile.days)}d`:(ship.status==="orbiting"?"Waiting berth":"—"));
    setText("[data-route-burn]",profile?`Fuel ${formatNumber(profile.fuelRequired)} • Food ${formatNumber(profile.foodRequired)}`:(ship.status==="travelling"?`Fuel ${formatNumber(fuel)} • Food ${formatNumber(food)}`:"—"));
    root.querySelectorAll("[data-ship-action]").forEach(button=>{const action=button.dataset.shipAction;if(action==="cargo")button.disabled=!dockedHere;button.onclick=()=>{if(button.disabled)return;this._panelReturn="ship";if(action==="cargo"||action==="accommodation"||action==="route")this.shipPrep?.();else if(action==="star-map")this.starMap();else if(action==="fleet")this.fleetManager?.();else if(action==="status"){if(dockedHere)this.shipPrep?.();else this.toast(`${ship.name||"Ship"} • ${status.label} • Cargo ${formatNumber(this.expansion.cargoAmount(this.state,ship.id))}/${formatNumber(ship.cargoCapacity)} • Fuel ${formatNumber(fuel)}/${formatNumber(ship.fuelCapacity)}.`);}};});
    const launch=root.querySelector("[data-ship-launch]");
    if(launch){
      if(ship.status==="travelling"){launch.disabled=true;launch.textContent="IN TRANSIT";}
      else if(ship.status==="orbiting"){launch.disabled=false;launch.textContent="REQUEST LANDING";launch.onclick=()=>{const colonyId=ship.targetColonyId||this.state.colonyId,result=this.expansion.dockAtColony(this.state,colonyId,ship.id);if(!result.ok){this.toast(result.reason);if(result.orbiting)this.repo.save(this.state);return;}if(this.onSwitchColony&&colonyId!==this.state.colonyId)this.onSwitchColony(colonyId);this.repo.save(this.state);this.toast(`Docked at ${this.state.contract?.colonyName||"colony"}.`);this.playerShipPanel();};}
      else{const canLaunch=this.expansion.canLaunch?.(this.state,ship.id)||{ok:false,launchEnabled:false,reason:"Launch unavailable."};launch.disabled=!(canLaunch.ok||canLaunch.launchEnabled);launch.textContent=canLaunch.ok?`LAUNCH • ${formatNumber(profile?.days||0)} DAYS`:esc(canLaunch.reason||"LAUNCH");launch.onclick=()=>{if(launch.disabled)return;if(typeof this.launchPreparedShip==="function")this.launchPreparedShip();else{const result=this.expansion.launch(this.state,ship.id);if(!result.ok){this.toast(result.reason);return;}this.onCapturePortfolio?.();this.repo.save(this.state);this.modal.classList.add("hidden");this.toast(`${ship.name} launched.`);}};}}
  }
  ...
  renderAdaptiveBuilding(tile){super.renderAdaptiveBuilding(tile);const root=this.adaptiveActionRoot?.()||this.modal?.querySelector("[data-adaptive-building-shell]");const demolish=root?.querySelector("[data-adaptive-demolish]");if(demolish)demolish.onclick=()=>this.demolitionPanel(tile);const check=this.colony.canStopProduction(tile);if(!check.ok||!root)return;root.querySelector("[data-production-toggle]")?.remove();const stopped=tile.development?.kind==="extract"?tile.productionStopped===true:tile.development?.productionStopped===true,button=document.createElement("button");button.dataset.productionToggle="1";button.textContent=stopped?"START PRODUCTION":"STOP PRODUCTION";button.classList.toggle("warn",stopped);const actions=root.querySelector(".adaptive-building-actions");if(demolish)demolish.before(button);else if(actions)actions.prepend(button);else root.append(button);button.onclick=()=>{const result=this.colony.setProductionStopped(this.state,tile,!stopped);if(!result.ok){this.toast(result.reason);return;}this.onRecalculate?.();this.repo.save(this.state);this.gameLog?.event?.(this.state,"production-toggle",`${tile.name||tile.development?.kind||"Building"} production ${result.stopped?"stopped":"started"}.`,{x:tile.x,y:tile.y,stopped:result.stopped});this.toast(result.stopped?"Production stopped. Operating load released.":"Production started.");this.render();this.renderAdaptiveBuilding(tile);};}

  async demolitionPanel(tile){
    const revision=++this.demolitionRevision,dev=tile?.development;if(!dev)return;
    const extract=dev.kind==="extract",label=extract?this.buildingLabel(dev):this.development.label(dev.kind),level=Math.max(1,Number(dev.level??tile.level)||1),investedBuild=Math.max(0,Math.floor(Number(dev.investedBuild)||0)),investedOre=Math.max(0,Math.floor(Number(dev.investedOre)||0)),recoverBuild=Math.floor(investedBuild*.25),recoverOre=Math.floor(investedOre*.25);
    ...
    const preview=this.development?.demolishPreview?.(this.state,tile);
    if(preview?.primary)impact=`Primary Headquarters will be cleared. Command capacity ${formatNumber(preview.capacity)} → ${formatNumber(preview.projectedCapacity)}; efficiency ${Math.round((preview.efficiency||0)*100)}% → ${Math.round((preview.projectedEfficiency||0)*100)}%.`;
    let body;try{body=await renderViewTemplate("./views/demolition-confirm.html",{BUILDING_LABEL:esc(label),LEVEL:level,X:tile.x,Y:tile.y,INVESTED_BUILD:formatNumber(investedBuild),RECOVER_BUILD:formatNumber(recoverBuild),INVESTED_ORE:formatNumber(investedOre),RECOVER_ORE:formatNumber(recoverOre),IMPACT:esc(impact)});}catch(error){if(revision!==this.demolitionRevision)return;this.diagnostics?.error?.("demolition template failed",error);this.toast("Unable to open demolition confirmation.");return;}
    if(revision!==this.demolitionRevision)return;this.open("Confirm Demolition",body);this.modal.classList.add("demolition-confirm-modal","full-screen-panel");
    this.modal.querySelector("[data-demolish-no]").onclick=()=>this.modal.classList.add("hidden");
    this.modal.querySelector("[data-demolish-yes]").onclick=()=>{this.modal.classList.add("hidden");this.onDemolishDevelopment?.(tile);};
  }
}
