...
export class PortfolioUIMixin {
  colonyStatus(summary){
    if(summary.status==="dead")return{label:"DEAD",cls:"bad"};
    if(summary.emergencyMode)return{label:"EMERGENCY",cls:"warn"};
    if(summary.survivalSupply<.4)return{label:"COLLAPSE",cls:"bad"};
    if(summary.survivalSupply<.9)return{label:"STRAINED",cls:"warn"};
  ...
  coloniesPanel(){
    ...
    const cards=entries.map(entry=>{const s=this.portfolio.summary(entry),st=this.colonyStatus(s);return`<button class="colony-card ${entry.id===active?"active":""}" data-colony-id="${entry.id}"><div class="colony-card-top"><strong>${s.name}</strong><span class="${st.cls}">${st.label}</span></div><div class="colony-card-env">T${s.tier} • ${s.environment} • support ×${Number(s.supportLoad||1).toFixed(2)}</div><div class="colony-card-grid"><span>POP<strong>${formatNumber(s.pop)}</strong></span><span>IND<strong>L${s.industryLevel} • ${Math.round((s.industryStaffFactor??1)*100)}%</strong></span><span>COST/D<strong>${formatMoney(s.operatingCost)}</strong></span><span>YEAR<strong>${s.year}</strong></span></div><div class="colony-card-stock"><span>F ${s.status==="dead"?formatNumber(s.foodStock):this.colonyDays(s.foodDays)}</span><span>FU ${s.status==="dead"?formatNumber(s.fuelStock):this.colonyDays(s.fuelDays)}</span><span>O ${s.status==="dead"?formatNumber(s.oreStock):this.colonyDays(s.oreDays)}</span></div>${entry.id===active?`<div class="colony-active-tag">CURRENTLY VIEWING</div>`:""}</button>`}).join("");
    const canNew=this.contracts.canOpenAdditional(this.state),living=entries.filter(entry=>entry.data?.status!=="dead").length;
    this.open(`Colonies — ${entries.length} (${living} active)`,`<div class="colony-grid">${cards}</div><button class="action" data-new-colony ${canNew?"":"disabled"}>OPEN NEW COLONY CONTRACT</button>${canNew?"":`<div class="requirement locked">${this.state.company.gameOver?"Corporation operations have ended because all colonies were lost.":"Complete the first 10-year contract to unlock additional colonies."}</div>`}`);
