import { WorldView as V591WorldView } from "./world-view-v591.js?v=5.9.1&legacy=1";
...
/**
 * v5.9.3 tile presentation.
 * Terrain is always the visual base. Survey state, resources and buildings are
 * progressively layered over it so the player never loses the geography.
 */
export class WorldView extends V591WorldView{
  ...
  drawResourceOverlay(c,tile,px,py,alpha=1){
    ...
    c.save();c.shadowColor="rgba(0,0,0,.75)";c.shadowBlur=Math.max(2,this.cell*.04);
    const drawn=this.icons.drawBackground?.(c,tile,x,y,size,alpha)===true;c.restore();
  ...
  drawResourceTile(c,tile,x,y,px,py,active,queued,selected){
    ...
    if(!hasBuilding){
      if(hasResource){
        const techLocked=!tile.depleted&&!this.technology.canExploit(this.state,tile);this.drawResourceOverlay(c,tile,px,py,techLocked?.48:tile.depleted?.30:.92);this.drawResourceInfo(c,tile,px,py);
