...
export class UIController extends LegacyUIController{
  ...
  setHidden(id,hidden){const el=document.querySelector(`#${id}`);if(el)el.hidden=!!hidden;}
  renderMapFirstHud(){
    const s=this.state,m=s.metrics||{},totals=syncBuildingTotals(s),ship=this.expansion.hudShip(s),planetary=this.expansion.planetaryResidentCount(s),builtHousing=Math.max(0,totals.builtHousing),power=Math.max(0,totals.builtPower),builtIndustry=Math.max(0,totals.builtIndustry),shipIndustry=ship?.id===s.colony?.foundingShipId?Math.max(0,totals.shipIndustry):0,industryFactor=Math.max(0,Math.min(1,Number(m.industryStaffFactor??1),Number(m.industryPowerFactor??1))),effectiveBuilt=builtIndustry*industryFactor,free=Math.max(0,Math.floor(Number(m.workforceFree)||0));
    this.setText("housingHud",`${formatNumber(planetary)} / ${formatNumber(builtHousing)}`);this.setText("powerHud",`${formatNumber(m.powerDelivered||m.powerFactor*(m.powerDemand||0)||0)} / ${formatNumber(m.powerFuelLimitedGeneration??power)} • ${formatNumber(m.powerDemand||0)}`);this.setText("industryHud",`${formatNumber(effectiveBuilt)} / ${formatNumber(builtIndustry)}`);this.setText("workforceHud",`${formatNumber(free)} FREE`);
    this.setState("housingColonyRow",builtHousing<=0&&planetary>0?"bad":builtHousing>0&&planetary/builtHousing>.9?"warn":"good");this.setState("powerColonyRow",(m.powerFactor??1)<.9?"bad":(m.powerFactor??1)<.98?"warn":"good");this.setState("industryColonyRow",(m.industryCommercialFactor??1)<.8?"bad":(m.industryCommercialFactor??1)<.999?"warn":"good");this.setState("workforceColonyRow",(m.workforceShortfall||0)>0?"bad":free<10?"warn":"good");
    for(const id of["housingShipRow","powerShipRow","industryShipRow","workforceShipRow"])this.setHidden(id,!ship);
    if(ship){const residents=this.expansion.shipResidentCount(s,ship.id),capacity=this.expansion.accommodationCapacity(s,ship.id),crew=Math.max(0,Number(ship.crew)||0),minimum=Math.max(0,Number(ship.minimumCrew)||0);this.setText("housingShipHud",`${formatNumber(residents)} / ${formatNumber(capacity)}`);this.setText("powerShipHud","SELF");this.setText("industryShipHud",`+${formatNumber(shipIndustry)}`);this.setText("workforceShipHud",`${formatNumber(crew)} / ${formatNumber(minimum)}`);this.setState("housingShipRow",capacity>0&&residents/capacity>.9?"warn":"good");this.setState("powerShipRow","good");this.setState("industryShipRow",shipIndustry>0?"good":"warn");this.setState("workforceShipRow",crew<minimum?"bad":"good");}
  ...
  attentionStatus(){
    ...
    const shipFood=this.expansion.shipResidentFoodStatus(s),shortest=shipFood.shortest;
    if(shortest&&shortest.daysRemaining<=30){const days=Math.max(0,Math.floor(shortest.daysRemaining)),critical=shortest.daysRemaining<10;if(days<=0){const grace=Math.max(0,30-shortest.starvationDays);return{level:"bad",title:grace>0?`SHIP RESIDENTS STARVING — DEATHS IN ${grace}d`:"SHIP RESIDENT DEATHS ACTIVE",detail:`${shortest.shipName} has no Food for ${formatNumber(shortest.residents)} residents.`,label:"OPEN SHIP ›",action:"player-ship"};}return{level:critical?"bad":"warn",title:`SHIP FOOD ${critical?"CRITICAL":"LOW"} — ${days}d`,detail:`${shortest.shipName} has the shortest occupied-ship Food runway. Colony Food is separate.`,label:"OPEN SHIP ›",action:"player-ship"};}
  ...
  runAttentionAction(){const a=this.currentAttention||this.attentionStatus();if(a.focus){this.focusMap(a.focus);return;}if(a.action==="ship"){document.querySelector("#tradeBtn")?.click();return;}if(a.action==="player-ship"){this.playerShipPanel?.();return;}if(a.action==="land-selection"){this.landSelection?.();return;}if(a.action==="establishment"){this.colonyEstablishment?.();return;}if(a.action==="colony"){(this.colonyControl?.()??this.landColonyPanel());}}
  ...
  contextParts(tile){
    ...
    if(this.land.isShipTile(tile.x,tile.y)){const ship=this.expansion.hudShip(this.state),accommodation=Math.max(0,Number(ship?.accommodationCapacity)||0),industry=this.state.colony?.shipIndustry||0;return{title:"LANDED SHIP",sub:`${formatNumber(accommodation)} accommodation • self-powered • ${formatNumber(industry)} Industry while docked`,actions:this.action("SHIP CONTROL","player-ship")+this.action("ESTABLISHMENT","establishment")+this.action("COLONY SUMMARY","colony"),requirement:""};}
  ...
  runContextAction(action,kind=null){
    const tile=this.selectedTile;if(action==="noop")return;if(action==="focus"){this.focusMap(kind||"all");return;}if(action==="tech"){this.tech();return;}if(action==="company"){this.company();return;}if(action==="player-ship"){this.playerShipPanel?.();return;}if(action==="establishment"){this.colonyEstablishment?.();return;}if(action==="colony"){(this.colonyControl?.()??this.landColonyPanel());return;}if(action==="details"){if(!tile)return;if(this.land.isShipTile(tile.x,tile.y))this.playerShipPanel?.();else if(tile.development&&LOCAL_KINDS.has(tile.development.kind))this.localBuildingPanel(tile);else if(tile.resourceId)this.tile(tile);else this.landTile(tile);return;}if(action==="mode"){this.tile(tile);return;}
