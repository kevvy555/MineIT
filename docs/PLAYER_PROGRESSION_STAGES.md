...
The detailed feature and bug backlog for these stages is maintained in [ProgressionStagesBacklog.md](./ProgressionStagesBacklog.md).
...
**Implementation progress:** Core second-colony expedition, accommodation and emergency ship-Food rules are complete. Active work on `feature/fleet-colony-ship-control` separates Colony Control from Ship Control (**N04**) and implements global ship management (**B03a**). Operational Headquarters departure gating (**A08**) remains ready for review and depends on that command split.
...
**Solution:** Purchase and operate factory-new specialised freight ships with the right capacity, range, speed and efficiency for the required routes. Player-designed and player-built ships become available later in the progression.  
**Implementation progress:** The physical-support foundation for this stage is implemented: colony-specific capability deployment through conglomerate Engineering Ships, fixed Engineering Ship transport plus individual package pricing, same-day batching, five-day preparation, Spaceport berths, orbital holding, and separate Mining/Scanning capability. Scanning records the equipment level used, hides resources beyond current detection, allows 50%-time resurvey after better Scanning is commissioned, and preserves buried-resource land-use decisions without leaking hidden deposits. The completed **Conglomerate Buyers Service** now adds the intermediate commercial/logistics layer: a deterministic 1,000-buyer catalogue, reputation-gated recurring contracts, quality/quantity/cadence terms, persistent buyer happiness/history, explicit full/partial/late/missed fulfilment, buyer-owned collection ships with capacities independent of the ordinary Corporate Ship, and the same canonical Spaceport berth/orbital-holding rules. Buyer collection events pause corporation time and are recovered across saves/background colonies. Factory-new ship procurement and minimum multi-ship fleet operation are also present; active work on `feature/fleet-colony-ship-control` covers **N03** multi-ship selection/access so purchased vessels remain operable beside the starter ship. The stage remains **In Progress** because scalable freight routing, advanced fleet dispatch and the broader logistics network are not yet complete. Player ship construction is intentionally reserved for later progression.
