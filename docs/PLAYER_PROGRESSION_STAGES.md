...
**Implementation progress:** Core second-colony expedition, accommodation and emergency ship-Food rules are complete. **A08a** and **A08b** (Headquarters command source, departure gate, outage/recovery, conglomerate network lockouts) are Complete. Discovery for Colony/Ship Control surfaces (**N04**) and global Fleet Manager (**B03a**) is Ready for Review on `feature/fleet-colony-ship-control` (with Fleet Manager mockup), re-checked after the A08b merge.
