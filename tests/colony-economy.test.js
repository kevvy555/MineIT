...
engine.tick(state);const rowsAfter=collection.current(state).filter(row=>row.tile?.resourceId);console.log("A08A DEBUG ECON",JSON.stringify({stock:rowsAfter[0]?.stock,rate:resources.collectionRate(state,stone),metrics:{workforceAvailable:state.metrics.workforceAvailable,workforceCommercialFactor:state.metrics.workforceCommercialFactor,industryCommercialFactor:state.metrics.industryCommercialFactor,powerFactor:state.metrics.powerFactor,commandEfficiency:state.metrics.commandEfficiency,powerFactors:state.metrics.powerFactors},stone:{developed:stone.developed,level:stone.level,development:stone.development,depleted:stone.depleted,resourceId:stone.resourceId}}));assert.ok(rowsAfter[0].stock>0,"collection popup stock should rise after collection");
