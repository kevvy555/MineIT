...
const world=new WorldService(resources,contracts),collection=new CollectionService(resources,inventory,tech),colony=new ColonyService(inventory,tech),sites=new SiteService(contracts,tech,inventory),trade=new TradeService(resources,inventory),engine=new SimulationEngine(resources,tech,collection,trade,inventory,colony);
const state=createGameState(contracts.first());tech.recompute(state);engine.recalculate(state);
assert.deepEqual(state.company.tech,{power:1,food:1,mining:1});
...
const stone={x:1,y:1,revealed:true,developed:false,depleted:false,type:"build",resourceId:"stone",name:"Stone",quality:100,resourceMult:1,requiredMiningLevel:2,requiredMiningTech:"Quarrying",sustainability:"finite",reserve:100000,initialReserve:100000};
...
const buildBefore=inventory.amount(state,"build");const developed=sites.develop(state,stone);assert.equal(developed.ok,true);assert.ok(inventory.amount(state,"build")<buildBefore);
...
assert.ok(inventory.amount(state,"ore")<oreBefore,"industry consumes ore");
assert.ok(state.metrics.foodDemand>0&&state.metrics.fuelDemand>0&&state.metrics.oreDemand>0);

const d1=colony.demand(state,collection.activeSites(state).length);state.pop*=2;state.colony.industryLevel+=2;const d2=colony.demand(state,collection.activeSites(state).length);assert.ok(d2.foodDemand>d1.foodDemand&&d2.fuelDemand>d1.fuelDemand&&d2.oreDemand>d1.oreDemand);
state.pop/=2;state.colony.industryLevel-=2;

state.colony.industryLevel=state.metrics.powerIndustryCap;assert.equal(colony.canExpandIndustry(state).ok,false);state.company.cash=1e9;assert.equal(tech.buy(state,"power").ok,true);tech.recompute(state);assert.ok(state.metrics.powerIndustryCap>state.colony.industryLevel);
...
state.trade.active=true;const gold=trade.catalog().find(x=>x.resourceId==="gold");const iron=trade.catalog().find(x=>x.resourceId==="iron");assert.ok(trade.sellPrice(gold)>trade.sellPrice(iron));assert.ok(trade.buyPrice(gold)>trade.sellPrice(gold));
...
state.trade.active=false;state.year=1;state.day=181;state.trade.nextArrivalDay=CONFIG.FIRST_TRADE_DAY;assert.equal(trade.shouldArrive(state),true);

console.log("MineIT colony economy + technology smoke test passed",{categories:[...cats],tech:state.company.tech,foodDemand:state.metrics.foodDemand,fuelDemand:state.metrics.fuelDemand,oreDemand:state.metrics.oreDemand});
