...
const state=createGameState(contracts.first()),foundingShip=engine.expansion.ship(state);
for(const key of Object.keys(foundingShip.foodLots))engine.expansion.unloadFood(state,foundingShip.id,key,Infinity);
for(const key of Object.keys(foundingShip.fuelLots))engine.expansion.unloadFuel(state,foundingShip.id,key,Infinity);
for(const key of Object.keys(foundingShip.cargo))engine.expansion.unloadCargo(state,foundingShip.id,key,Infinity);
state.colony.shipAccommodation={};state.tiles["0,0"]={x:0,y:0,terrain:"plain",revealed:true,developed:true,name:"Power Plant",development:{kind:"power",level:1,investedBuild:40,investedOre:0}};state.tiles["0,1"]={x:0,y:1,terrain:"plain",revealed:true,developed:true,name:"Industry",development:{kind:"industry",level:1,investedBuild:80,investedOre:0}};tech.recompute(state);engine.recalculate(state);
