...
for(const type of["food","build","fuel","ore"]){assert.match(index,new RegExp(`id="${type}ResourceHud"`),`${type} resource card needs a neutral shell`);assert.match(index,new RegExp(`id="${type}ShipDaysHud"`),`${type} ship row needs days remaining`);assert.match(index,new RegExp(`id="${type}DaysHud"`),`${type} colony row needs days remaining`);assert.match(index,new RegExp(`id="${type}ShipStock"[^>]*>0 \\+0 -0 S0`),`${type} ship row should expose the compact flow format`);assert.match(index,new RegExp(`id="${type}Stock"[^>]*>0 \\+0 -0 S0`),`${type} colony row should expose the compact flow format`);}
assert.match(operational,/RESOURCE_FLOW_METRICS/);assert.match(operational,/build:\{production:"buildProduction",consumption:"buildDemand"\}/,"Build must participate in the same flow HUD even though its current daily demand is zero");assert.match(operational,/resourceFlowText\(flow\)/);assert.match(operational,/S\$\{sign\}/,"surplus must retain an explicit S prefix and sign");assert.match(operational,/resourceDaysText\(days\).*∞d/);assert.match(operational,/colonyRow\.classList\.toggle\("good",!flow\.declining\)/);assert.match(operational,/colonyRow\.classList\.toggle\("bad",flow\.declining\)/);assert.match(operational,/tone=shipFlow\.days===null\|\|shipFlow\.days>=10\?"good":"bad"/);assert.match(mapFirstCss,/\.resource-flow-row\.good\{background:#102919\}/);assert.match(mapFirstCss,/\.resource-flow-row\.bad\{background:#3a1416\}/);assert.match(mapFirstCss,/font-variant-numeric:tabular-nums/);
