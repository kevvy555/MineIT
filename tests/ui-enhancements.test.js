...
const ui=read("js/ui/ui-enhancements.js"),helpView=read("views/help-manual.html"),viewTemplate=read("js/core/view-template.js");for(const key of["name","category","rate","stock","remaining"])assert.ok(ui.includes(`[\"${key}\"`)||ui.includes(`\"${key}\"`),`Collect sorting must include ${key}`);assert.match(ui,/data-collection-sort/);assert.match(ui,/collectionSort\.dir\*=-1/);
...
for(const marker of["MINEIT FIELD MANUAL","Objective &amp; game loop","Resource catalogue","Quality, production &amp; sale value","Corporate trade ship","Multiple colonies","Returning colonies, refusal &amp; relocation","Saving, reset &amp; diagnostics"])assert.ok(helpView.includes(marker),`Detailed help view missing ${marker}`);assert.match(ui,/this\.resources\.catalog\(\)/,"Manual resource list must remain data-driven");assert.match(ui,/renderViewTemplate\("\.\/views\/help-manual\.html"/);assert.match(viewTemplate,/templateCache/);assert.match(helpView,/data-help-target/);assert.match(helpView,/help-index/);assert.doesNotMatch(ui,/MINEIT FIELD MANUAL/,"Static field-manual markup belongs in the external view, not the UI controller");
