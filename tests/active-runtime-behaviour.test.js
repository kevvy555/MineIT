...
rescanState.tiles["2,2"]=rescanTile;engine.recalculate(rescanState);console.log("A08A DEBUG RESCAN",JSON.stringify({rate:resources.collectionRate(rescanState,rescanTile),metrics:rescanState.metrics,colony:rescanState.colony,power:rescanState.metrics.powerBands}));const rateBeforeRescan=resources.collectionRate(rescanState,rescanTile);assert.ok(rateBeforeRescan>0,"developed extraction must produce before resurvey");
