...
commandExpansion.colonyService=commandColony;commandState.colony.shipAccommodation={};
for(const type of["build","ore","fuel"])commandInventory.store(commandState,type,type==="build"?"fiber":type==="ore"?"surface-iron":"biomass","Test",100000,500);
commandState.tiles.power={x:0,y:1,terrain:"plain",revealed:true,development:{kind:"power",level:1}};
commandState.tiles["1,1"]={x:1,y:1,terrain:"plain",revealed:true,development:{kind:"headquarters",level:1}};
commandState.tiles["2,1"]={x:2,y:1,terrain:"plain",revealed:true,development:{kind:"headquarters",level:2}};
commandColony.totals(commandState);
let command=commandColony.headquartersNetwork(commandState);
assert.equal(command.primaryId,"1,1");assert.equal(command.source?.type,"headquarters");assert.equal(command.capacity,52);
assert.equal(commandDevelopment.demolish(commandState,commandState.tiles["1,1"]).ok,true);
command=commandColony.headquartersNetwork(commandState);
assert.equal(command.source?.type,"ship");assert.equal(command.capacity,52);assert.equal(command.rows.find(row=>row.id==="2,1").staffed,true);assert.equal(command.rows.find(row=>row.id==="2,1").eligibleForPrimary,true);assert.equal(command.reserved,10);assert.equal(command.power.bandRows.find(row=>row.priority==="headquarters").requested,2);
const recoveredState=JSON.parse(JSON.stringify(commandState));assert.equal(commandColony.setPrimaryHeadquarters(recoveredState,recoveredState.tiles["2,1"]).ok,true);assert.equal(commandColony.headquartersNetwork(recoveredState).source?.type,"headquarters");
const commandShip=commandExpansion.ship(commandState);commandShip.crew=commandShip.minimumCrew;command=commandColony.headquartersNetwork(commandState);assert.equal(command.source?.type,"ship");assert.equal(command.capacity,52);assert.equal(command.rows.find(row=>row.id==="2,1").staffed,true);assert.equal(command.power.bandRows.find(row=>row.priority==="headquarters").requested,2);

const shipSupportState=createGameState(contracts.first()),shipSupportInventory=new InventoryService(resources),shipSupportColony=new ColonyService(shipSupportInventory,{}),shipSupportTechnology={recompute(){},processDay(){return[];}},shipSupportCollection={activeSites(){return[];},shutdownSites(){return[];},collectDay(){return{collected:0};},advanceShutdowns(){}},shipSupportEngine=new SimulationEngine(resources,shipSupportTechnology,shipSupportCollection,{stockValue:()=>0},shipSupportInventory,shipSupportColony);
const supportedShip=shipSupportEngine.expansion.ship(shipSupportState),shipFoodKey="food:fungal",shipFoodEntry=supportedShip.foodLots[shipFoodKey],shipFoodBefore=shipSupportEngine.expansion.transitFoodAmount(shipSupportState,supportedShip.id),populationBefore=shipSupportState.pop,shipDay=shipSupportEngine.tick(shipSupportState);
assert.equal(shipSupportColony.foodDemandForPopulation(shipSupportState),0);assert.equal(shipDay.deaths,0);assert.equal(shipSupportState.pop,populationBefore);assert.ok(shipSupportEngine.expansion.transitFoodAmount(shipSupportState,supportedShip.id)<shipFoodBefore);assert.equal(shipSupportState.metrics.shipFoodConsumed,populationBefore*CONFIG.FOOD_PER_COLONIST);
...
const adjState=createGameState(contracts.first()),adjInventory=new InventoryService(resources),adjColony=new ColonyService(adjInventory,{}),adjDevelopment=new DevelopmentService(adjInventory,new LandService(),adjColony);
for(const type of["build","ore","fuel"])adjInventory.store(adjState,type,type==="build"?"fiber":type==="ore"?"surface-iron":"biomass","Test",100000,500);
adjState.tiles["5,5"]={x:5,y:5,terrain:"plain",revealed:true,resourceId:null,developed:false};
assert.equal(adjDevelopment.place(adjState,adjState.tiles["5,5"],"headquarters").ok,true,"first Headquarters may be placed anywhere surveyed");
adjState.colony.primaryHeadquartersId="5,5";adjState.colony.primaryHeadquartersEver=true;
adjState.tiles["7,7"]={x:7,y:7,terrain:"plain",revealed:true,resourceId:null,developed:false};
assert.equal(adjDevelopment.canPlace(adjState,adjState.tiles["7,7"],"headquarters").ok,false,"non-adjacent Headquarters expansion must be blocked");
assert.match(adjDevelopment.canPlace(adjState,adjState.tiles["7,7"],"headquarters").reason,/adjacent/i);
adjState.tiles["6,5"]={x:6,y:5,terrain:"plain",revealed:true,resourceId:null,developed:false};
assert.equal(adjDevelopment.canPlace(adjState,adjState.tiles["6,5"],"headquarters").ok,true,"adjacent Headquarters expansion must remain allowed");
assert.equal(adjDevelopment.place(adjState,adjState.tiles["6,5"],"headquarters").ok,true);
