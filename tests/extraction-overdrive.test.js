...
assert.equal(OPERATING_MODES.pushed.workforce,1.25);assert.equal(OPERATING_MODES.pushed.output,1.15);assert.equal(OPERATING_MODES.hard.workforce,1.5);assert.equal(OPERATING_MODES.hard.output,1.3);
...
assert.equal(supportsOverdrive(tile("farm","food")),false,"food sites must remain unchanged");const renewable=tile("mine","fuel");renewable.sustainability="renewable";assert.equal(supportsOverdrive(renewable),false,"renewable sites keep their existing harvest system");

const mine=tile("mine","ore"),resources=new ResourceService(),state={metrics:{workforceCommercialFactor:1,industryCommercialFactor:1}};const normalOutput=resources.sitePotentialRate(mine);assert.ok(setOperatingMode(mine,"hard").ok);assert.equal(resources.sitePotentialRate(mine),normalOutput*1.3);
const colony=new ColonyService({},{}),workerState={metrics:{miningWorkforceEfficiency:1}};const normalWorkers=colony.siteWorkforce(workerState,mine,null,"normal"),hardWorkers=colony.siteWorkforce(workerState,mine,null,"hard");assert.equal(hardWorkers,Math.ceil(normalWorkers*1.5));
...
console.log("canonical extraction overdrive and accident tests passed");
