...
const activeWorld=read("js/ui/world-view-v5112.js"),activeUi=read("js/ui/ui-controller-v5113.js"),activeTrade=read("js/ui/quick-trade-ui-v5111.js"),baseControls=read("js/ui/map-controls.js"),activeControls=read("js/ui/map-controls-v581.js"),resourceIcons=read("js/ui/resource-icons-v597.js"),baseUi=read("js/ui/ui-controller.js");
assert.match(activeWorld,/mineit:player-ship-clicked/);assert.match(activeWorld,/stopImmediatePropagation/);assert.match(activeUi,/planetSortHeader/);assert.match(activeUi,/availableCargoCategories/);assert.match(activeUi,/manifestOverview/);assert.match(activeTrade,/CORP SHIP • TRADE!/);assert.match(baseControls,/replaceChildren/);assert.match(baseControls,/map-toolbar-main/);assert.match(activeControls,/mineit:map-focus/);assert.match(resourceIcons,/resource-atlas-256\.webp/);assert.match(baseUi,/export function mix/);assert.match(baseUi,/Object\.getPrototypeOf/);
for(const [name,source] of Object.entries({activeWorld,activeUi,activeTrade,baseControls,activeControls,resourceIcons,baseUi})){assert.doesNotMatch(source,/prototype\.[A-Za-z0-9_$]+\s*=/,`${name} must not monkey-patch prototypes`);assert.doesNotMatch(source,/createElement\(["']style["']\)/,`${name} must not inject runtime styles`);}
