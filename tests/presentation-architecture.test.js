...
assert.doesNotMatch(index,/map-controls-v58/,"map controls must resolve directly to the canonical module");assert.match(index,/corporate-trade-ui\.js/);assert.doesNotMatch(index,/quick-trade-ui-v511/,"corporate trade must resolve through the semantic adapter");assert.doesNotMatch(index,/ship-expansion-v5113\.css/,"ShipExpansion styles must be consolidated");
...
const activeWorld=read("js/ui/world-view-v5112.js"),activeUi=read("js/ui/ui-controller-v5113.js"),activeTrade=read("js/ui/corporate-trade-ui.js"),controls=read("js/ui/map-controls.js"),resourceIcons=read("js/ui/resource-icons-v597.js"),baseUi=read("js/ui/ui-controller.js");
assert.match(activeWorld,/mineit:player-ship-clicked/);assert.match(activeWorld,/stopImmediatePropagation/);assert.match(activeUi,/planetSortHeader/);assert.match(activeUi,/availableCargoCategories/);assert.match(activeUi,/manifestOverview/);assert.match(activeTrade,/CORP SHIP • TRADE!/);assert.match(activeTrade,/extends QuickTradeUI/);assert.match(controls,/replaceChildren/);assert.match(controls,/map-toolbar-main/);assert.match(controls,/mineit:map-focus/);assert.match(controls,/dispose\(\)/);assert.match(resourceIcons,/resource-atlas-256\.webp/);assert.match(baseUi,/export function mix/);assert.match(baseUi,/Object\.getPrototypeOf/);
