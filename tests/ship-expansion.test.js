...
const uiSource=readFileSync(new URL("../js/ui/ui-controller-v511.js",import.meta.url),"utf8"),ui5111=readFileSync(new URL("../js/ui/ui-controller-v5111.js",import.meta.url),"utf8"),ui5112=readFileSync(new URL("../js/ui/ui-controller-v5112.js",import.meta.url),"utf8"),ui5113=readFileSync(new URL("../js/ui/ui-controller-v5113.js",import.meta.url),"utf8"),view5111=readFileSync(new URL("../js/ui/world-view-v5111.js",import.meta.url),"utf8"),view5112=readFileSync(new URL("../js/ui/world-view-v5112.js",import.meta.url),"utf8"),tradeUi5111=readFileSync(new URL("../js/ui/quick-trade-ui-v5111.js",import.meta.url),"utf8"),css=readFileSync(new URL("../css/ship-expansion.css",import.meta.url),"utf8"),css5113=readFileSync(new URL("../css/ship-expansion-v5113.css",import.meta.url),"utf8"),simulation5113=readFileSync(new URL("../js/domain/simulation-engine-v5113.js",import.meta.url),"utf8"),serviceSource=readFileSync(new URL("../js/domain/expansion-service.js",import.meta.url),"utf8");
for(const marker of["starMapCanvas","BUILD + LAUNCH SURVEY PROBE","AS DESTINATION","SHIP FUEL TANK","Koplin Corporate Home"])assert.ok(uiSource.includes(marker));
for(const marker of["TECHNOLOGY","STAR MAP","COLONIES","CARGO BAY","COLONY SUMMARY","CORPORATION","colonyNavStrip"])assert.ok(ui5111.includes(marker));
for(const marker of["full-screen-panel","starSystemDetailMarkup","exp-planet-table","selectedStarSystemId","ship-panel-back","danger-close","demolitionPanel","YES • DEMOLISH","NO • KEEP BUILDING"])assert.ok(ui5112.includes(marker),`missing v5.11.2 UI marker ${marker}`);
for(const marker of["planetSortHeader","data-planet-sort","availableCargoCategories","colony>0||aboard>0","SHIP LOAD","SHIP FUEL TANK","TRANSIT FUEL","TRANSIT FOOD","CARGO FOOD","exp-resource-load","compact-ship-prep"])assert.ok(ui5113.includes(marker),`missing v5.11.3 UI marker ${marker}`);
assert.doesNotMatch(ui5112,/confirm\(/,"active v5.11.2 demolition path must not use a browser confirmation dialog");
assert.ok(view5111.includes('ship.status==="docked"'));assert.ok(view5112.includes("mineit:player-ship-clicked"));assert.ok(view5112.includes("stopImmediatePropagation"));assert.ok(tradeUi5111.includes("CORP SHIP • TRADE!"));
for(const marker of[".full-screen-panel",".exp-planet-table",".demolition-confirm",".demolition-yes",".demolition-no"])assert.ok(css.includes(marker),`missing v5.11.2 CSS marker ${marker}`);
for(const marker of[".exp-planet-sort",".compact-ship-prep",".exp-load-overview",".exp-load-meter",".exp-resource-load"])assert.ok(css5113.includes(marker),`missing v5.11.3 CSS marker ${marker}`);
assert.ok(simulation5113.includes("siteEnded"));assert.ok(simulation5113.includes("updateNetworks(state,active)"),"depleted sites must refresh workforce allocation in the same tick");
for(const marker of["CORPORATE_SERVICE_RADIUS_LY","probe-en-route","awaitingDestination","sellCargoAtHome"])assert.ok(serviceSource.includes(marker));
console.log("MineIT ShipExpansion v5.11.3 regression test passed");
