...
const uiSource=readFileSync(new URL("../js/ui/ui-controller-v511.js",import.meta.url),"utf8"),ui5111=readFileSync(new URL("../js/ui/ui-controller-v5111.js",import.meta.url),"utf8"),view5111=readFileSync(new URL("../js/ui/world-view-v5111.js",import.meta.url),"utf8"),tradeUi5111=readFileSync(new URL("../js/ui/quick-trade-ui-v5111.js",import.meta.url),"utf8"),serviceSource=readFileSync(new URL("../js/domain/expansion-service.js",import.meta.url),"utf8");for(const marker of["starMapCanvas","BUILD + LAUNCH SURVEY PROBE","AS DESTINATION","SHIP FUEL TANK","Koplin Corporate Home"])assert.ok(uiSource.includes(marker));for(const marker of["TECHNOLOGY","STAR MAP","COLONIES","CARGO BAY","COLONY SUMMARY","CORPORATION","colonyNavStrip"])assert.ok(ui5111.includes(marker));assert.ok(view5111.includes('ship.status==="docked"'));assert.ok(tradeUi5111.includes("CORP SHIP • TRADE!"));for(const marker of["CORPORATE_SERVICE_RADIUS_LY","probe-en-route","awaitingDestination","sellCargoAtHome"])assert.ok(serviceSource.includes(marker));
console.log("MineIT ShipExpansion v5.11.1 regression test passed");