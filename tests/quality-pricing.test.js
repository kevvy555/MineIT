...
const state={inventory:{},company:{cash:0,earn:0},contract:{localRevenue:0,localCosts:0},trade:{active:true,cargoUsed:0}};
...
assert.equal(trade.sellPrice("ore","gold","common"),18.75);assert.equal(trade.sellPrice("ore","gold","excellent"),25);assert.equal(trade.sellPrice("ore","gold","extraordinary"),75);
assert.equal(trade.stockValue(state),3375,"Stock value must use band-specific prices");
const sale=trade.sellBand(state,"ore:gold","common",40);assert.equal(sale.revenue,750);assert.equal(gold.qualityBands.common.amount,60);assert.equal(gold.qualityBands.extraordinary.amount,20);assert.equal(gold.amount,80);
