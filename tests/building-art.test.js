...
const art=read("js/ui/land-art.js"),world=read("js/ui/world-view-runtime.js"),buildingUi=read("js/ui/building-details-ui.js"),detailsCss=read("css/building-details.css"),pkg=JSON.parse(read("package.json"));
...
assert.match(buildingUi,/from "\.\/land-art\.js"/);assert.doesNotMatch(buildingUi,/land-art\.js\?v=/);assert.match(buildingUi,/developmentOriginalPath/);assert.match(buildingUi,/buildingHero\(dev/);assert.match(buildingUi,/openColonyBuilding\(tile\)/);assert.match(buildingUi,/compactExtractionPanel\(tile\)/);assert.match(buildingUi,/UPGRADE TO L/);assert.match(buildingUi,/data-building-demolish/);assert.match(detailsCss,/\.building-art-frame img/);assert.match(detailsCss,/object-fit:contain/);assert.match(detailsCss,/\.building-detail-actions/);
