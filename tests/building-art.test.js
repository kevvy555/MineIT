...
assert.match(buildingUi,/from "\.\/land-art\.js\?v=5\.9\.1"/);assert.match(buildingUi,/developmentOriginalPath/);assert.match(buildingUi,/buildingHero\(dev/);assert.match(buildingUi,/openColonyBuilding\(tile\)/);assert.match(buildingUi,/compactExtractionPanel\(tile\)/);assert.match(buildingUi,/UPGRADE TO L/);assert.match(buildingUi,/data-building-demolish/);assert.match(detailsCss,/\.building-art-frame img/);assert.match(detailsCss,/object-fit:contain/);assert.match(detailsCss,/\.building-detail-actions/);
