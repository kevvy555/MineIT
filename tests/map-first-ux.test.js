...
const pkg=JSON.parse(read("package.json")),index=read("index.html"),controls=read("js/ui/map-controls-v580.js"),view=read("js/ui/world-view-v580.js"),ui=read("js/ui/ui-controller-v580.js"),css=read("css/map-first.css");
assert.equal(pkg.version,"5.8.0");
assert.match(index,/map-first\.css\?v=5\.8\.0/);assert.match(index,/map-controls-v580\.js\?v=5\.8\.0/);assert.match(index,/world-view-v580\.js\?v=5\.8\.0/);assert.match(index,/ui-controller-v580\.js\?v=5\.8\.0/);
...
assert.doesNotMatch(controls,/\[\["land","LAND"\],\["resource","RESOURCES"\]\]/,"active map controls must not expose two map modes");assert.match(controls,/COLONY MAP/);assert.match(controls,/PROBLEMS/);assert.match(controls,/UPGRADE/);assert.match(controls,/mineit:map-focus/);
...
console.log("MineIT v5.8.0 map-first UX regression test passed");
