...
const hiddenTile=world.reveal(prospect,hidden.tile.x,hidden.tile.y,1),emptyTile=world.reveal(prospect,trulyEmpty.tile.x,trulyEmpty.tile.y,1);assert.equal(hiddenTile.revealed,true);assert.equal(hiddenTile.resourceId,null,"insufficient Scanning must look exactly like an ordinary clear result");assert.equal(hiddenTile.unresolved,false);assert.equal(hiddenTile.requiredScanningLevel,0);assert.equal(hiddenTile.lastScannedAtLevel,1);assert.doesNotMatch(world.hint(prospect,hiddenTile.x,hiddenTile.y),/required|anomaly|L\d/i,"visible scan copy must not leak the hidden detection requirement");assert.equal(emptyTile.resourceId,null);assert.equal(emptyTile.lastScannedAtLevel,1);
